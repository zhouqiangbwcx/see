package com.see.service.configure;
...
public class RabbitMQConfig {
	 ...
	 * fanout交换器中没有路由键的概念，他会把消息发送到所有绑定在此交换器上面的队列中。 topic交换器你采用模糊匹配路由键的原则进行转发消息到队列中
	 * key:
	 * queue在该direct-exchange中的key值，当消息发送给direct-exchange中指定key为设置值时，消息将会转发给queue参数指定
	 * 的消息队列
	 * 
	 * @date:2017/9/1
	 * @className:ConnectionFactory
	...
	public Binding binding_one() {
		return BindingBuilder.bind(queue_one()).to(directExchange()).with(RabbitMQConfig.ROUTINGKEY1);
	}
	...
	public Binding binding_two() {
		return BindingBuilder.bind(queue_two()).to(directExchange()).with(RabbitMQConfig.ROUTINGKEY2);
	}
	...
	public SimpleMessageListenerContainer messageContainer() {
		...
		container.setMessageListener(new ChannelAwareMessageListener() {
			...
			public void onMessage(Message message, com.rabbitmq.client.Channel channel) throws Exception {
				...
				System.out.print("消费端接收到消息 ssss: " + new String(body));
